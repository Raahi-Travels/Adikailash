...
So nothing here produces a rating out of five as its answer. `assess()` returns
evidence — a headline sentence, the concerns that must be read, the ratings where
there are enough of them to mean anything, and the incidents, which are never
averaged into anything. A four-point-two beside a homestay where a traveller was
left outside at night is a worse artefact than no number at all, because it gets
believed.
...
def assess(record: VendorRecord) -> VendorAssessment:
    """Turn a vendor's record into a recommendation and the reasons for it."""
    blocking: list[str] = []
    notes: list[str] = []
